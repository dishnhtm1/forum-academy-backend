...
router.post("/", async (req, res) => {
  try {
    ...
    console.log("✅ Contact saved successfully, notifying admins...");

    // Notify all admins about the new contact message
    try {
      await NotificationService.notifyAdminsContactSubmission(
        contact._id,
        contact.name,
        contact.email
      );
      console.log("✅ Admin notifications sent successfully for contact");
    } catch (notifError) {
      console.error(
        "⚠️ Error sending admin notifications for contact:",
        notifError
      );
    ...
    try {
      ...
      if (!contactId || !subject || !message || !recipientEmail) {
        return res.status(400).json({
          success: false,
          message:
            "Missing required fields: contactId, subject, message, recipientEmail",
        });
      }
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;">
            ...
            <div style="border-top: 1px solid #e5e7eb; padding-top: 15px; margin-top: 20px;">
                <p style="color: #6b7280; font-size: 14px; margin: 0;">
                    <strong>Reference:</strong> This is a reply to your inquiry: "${
                      contact.subject
                    }"
                </p>
                <p style="color: #6b7280; font-size: 14px; margin: 5px 0;">
                    <strong>Original message submitted:</strong> ${new Date(
                      contact.createdAt
                    ).toLocaleDateString()}
                </p>
            </div>
...
router.post(
  "/messages/send",
  authenticate,
  authorizeRoles("admin", "teacher", "faculty"),
  async (req, res) => {
    try {
